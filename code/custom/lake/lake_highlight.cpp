 ...
 * lake_highlight.cpp
 * ------------------------------------------------------------------------
 * Semantic identifier coloring for the "lake" custom layer using 4coder's
 * built-in Code_Index.
 *
 * ------------------------------------------------------------------------
 * INCLUDE-PROCESSING DESIGN (read this before touching that section)
 * ------------------------------------------------------------------------
 * Earlier version of this file rescanned every open buffer for #includes,
 * every time ANY buffer opened, in a `while (new_buffer_opened)` loop --
 * and resolved <angle-bracket> includes against the full MSVC STL +
 * Windows UCRT header folders. Since STL headers transitively include
 * dozens of other STL headers, and the "rescan everything" loop meant
 * every one of THOSE newly-opened headers triggered another full sweep of
 * every open buffer, a single `#include <iostream>` in the user's project
 * would snowball into opening and tokenizing a large fraction of the
 * standard library, effectively hanging the editor.
 *
 * Fixed by two structural changes:
 *
 *   1. WORKLIST, NOT RESCAN. New includes get pushed onto a bounded queue
 *      (lake_include_queue) and drained once. We never re-scan a buffer
 *      we've already scanned (lake_scanned_paths tracks that by resolved
 *      absolute path, not by "does a buffer with this name exist" -- the
 *      old check happened too late, after resolution/creation).
 *
 *   2. SYSTEM HEADERS ARE LEAVES. A <system> header gets opened and
 *      indexed (so e.g. jump-to-definition on `printf` still works) but
 *      its own #includes are never followed. Only project-local ("quoted")
 *      includes get recursed into. This is the actual fix for the
 *      explosion -- STL/CRT headers are exactly the ones with huge
 *      transitive include graphs, and lake has no real use for indexing
 *      libstdc++/MSVC-STL's own internals anyway.
 *
 *   A hard cap (LAKE_MAX_QUEUED_INCLUDES) is kept as a backstop even after
 *   those two fixes, specifically so a bug in the visited-path check can
 *   never again turn into an unbounded hang -- worst case you eat the cap
 *   and stop, instead of eating all your RAM.
 * ------------------------------------------------------------------------
 */
